...
async function translate(text, targetLang, apiKey, apiEndpoint, modelName) {
  ...
  try {
    ...
    if (errorMsg.includes('quota') || errorMsg.includes('insufficient_quota')) {
      ...
      return chrome.i18n.getMessage('errorInvalidApiKey') || '[Translation failed: Invalid API key]';
    } else if (errorMsg.includes('rate_limit') || errorMsg.includes('429')) {
      return chrome.i18n.getMessage('errorRateLimit') || '[Translation failed: Rate limit exceeded]';
      ...
      return chrome.i18n.getMessage('errorInvalidResponse') || '[Translation failed: Invalid API response]';
    } else {
      return `[${chrome.i18n.getMessage('errorTranslationFailed') || 'Translation failed'}: ${errorMsg}]`;
    }
  }
}
